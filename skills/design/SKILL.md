...
7. 用户只要求方案时，停止在设计产物；用户决定实现时，中大型任务进入 `plan`，小型明确任务可进入 `exec-plan`。
...
- 完成 `design.md` 写入后，提示用户下一步将加载 `plan` 生成计划任务文档；仅当后续实现已被判定为小型明确任务时，可提示进入 `exec-plan` 直接执行。
...
- 下一步明确指向 `plan`、小型明确任务的 `exec-plan`，或等待用户确认。

...
5. 判断是否涉及功能变更：
   * 不涉及功能变更的小型修改（如修复 typo、调整格式、更新依赖版本等），直接执行。
   * 不涉及功能变更但规模较大（如代码重构、性能优化、技术栈/框架替换变更等任务），需要加载 `design` 技能。
   * 涉及功能变更（新增、修改、删除功能行为）→ 需要加载 `design` 技能。
...
6. 路由图联动：
   ...
   - 路由图不存在且当前任务需要代码定位时，必须先加载 `route-init` 技能完成首次初始化；初始化后回到本技能继续 lookup，并将覆盖状态标记为 `partial`。
...
- 建议的下一步技能：`design` / `route-sync` / `route-init` / 无（直接执行）
...
- Next skill: design / exec-plan / route-sync / route-init / none
```

## 首次初始化

路由图不存在且当前任务需要代码定位时，必须加载 `route-init` 技能完成首次初始化。初始化完成后继续本技能的 lookup 流程，并将覆盖状态标记为 `partial`。
